...
strokeStart=3 # mm, первый ход клапана
...
cp -r /media/psf/HDD/Diploma/Drawings/intakePipe/version1/stl/*.stl 0/geometry
...
surfaceTransformPoints -scale '(0.001 0.001 0.001)' $FILE $FILE | tee -a ../../project.log
export FILE="inlet.stl"
surfaceTransformPoints -translate '(0.001 0 0)' $FILE $FILE | tee -a ../../project.log
...
do
	...
	cd - # Корневая директория проекта
	cd $stroke
	printf '\n  Stroke %s mm was solved in ' $stroke >> ../project.log
	endStrokeTime=`date +%s`
	solveStrokeTime=$((endStrokeTime-startStrokeTime))
	printf '%dh:%dm:%ds\n' $(($solveStrokeTime/3600)) $(($solveStrokeTime%3600/60)) $(($solveStrokeTime%60)) >> ../project.log
	
	stroke=`echo "scale=1; $stroke + $strokeDelta" |bc` # Inreasing stroke by one step
done

# Сохранение времени расчёта проекта в project.log
cd - # Корневая директория проекта
...
printf '\n/---------------------------------------------------------------------------------------/' | tee -a project.log
printf '\n\n                                   ' | tee -a project.log                            
printf 'Solve time: %dh:%dm:%ds\n' $(($solveProjectTime/3600)) $(($solveProjectTime%3600/60)) $(($solveProjectTime%60)) | tee -a project.log
printf '\n/---------------------------------------------------------------------------------------/' | tee -a project.log
printf '\n\n'